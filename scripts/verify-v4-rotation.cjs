 ...
 * Optional release identity: EXPECTED_MODEL_URL, EXPECTED_MODEL_BYTES,
 * EXPECTED_MODEL_SHA256. Supply these when validating a specific final asset.
...
const report = {
  ...
  errors: [],
  expectedModel: {
    url: process.env.EXPECTED_MODEL_URL || null,
    bytes: process.env.EXPECTED_MODEL_BYTES ? Number(process.env.EXPECTED_MODEL_BYTES) : null,
    sha256: process.env.EXPECTED_MODEL_SHA256?.toLowerCase() || null,
  },
};
...
for pair in config['pairs']:
    ...
    changed = sum(maximum.histogram()[threshold + 1:])
    result = dict(pair)
    result.update({
        'region': roi,
...
(async () => {
  ...
  for (const modelResponse of modelResponses) {
    const observedURL = modelResponse.url();
    assert.equal(modelResponse.status(), 200, 'The browser model response must be HTTP 200');
    let bytes;
    let fingerprintMethod = 'body of the actual browser response';
    let inspectorCacheError;
    try {
      bytes = await modelResponse.body();
    } catch (error) {
      // Large GLBs can be evicted from Chrome's inspector response cache even
      // after the scene has rendered successfully. Re-read the exact observed
      // resource URL through Playwright's HTTP client, retaining this distinction.
      inspectorCacheError = error.message || String(error);
      const fingerprintResponse = await page.request.get(observedURL);
      try {
        assert.equal(fingerprintResponse.status(), 200, 'The model fingerprint request must be HTTP 200');
        assert.equal(fingerprintResponse.url(), observedURL, 'Fingerprint must use the exact observed model URL');
        bytes = await fingerprintResponse.body();
      } finally {
        await fingerprintResponse.dispose();
      }
      fingerprintMethod = 'independent full HTTP GET to the exact browser-observed model URL after inspector response eviction';
    }
    report.modelAssets.push({
      ...
      fingerprintMethod,
      ...(inspectorCacheError ? { inspectorCacheError } : {}),
    });
  }
  check('Actual GLB response fingerprint recorded', report.modelAssets.length > 0, report.modelAssets);
  if (Object.values(report.expectedModel).some(value => value !== null)) {
    if (report.expectedModel.bytes !== null) assert(Number.isInteger(report.expectedModel.bytes) && report.expectedModel.bytes > 0, 'EXPECTED_MODEL_BYTES must be a positive integer');
    if (report.expectedModel.sha256 !== null) assert(/^[a-f0-9]{64}$/.test(report.expectedModel.sha256), 'EXPECTED_MODEL_SHA256 must contain 64 hexadecimal characters');
    check('Observed model matches the specified release URL, byte size and SHA256', report.modelAssets.some(asset =>
      (report.expectedModel.url === null || asset.url === report.expectedModel.url) &&
      (report.expectedModel.bytes === null || asset.bytes === report.expectedModel.bytes) &&
      (report.expectedModel.sha256 === null || asset.sha256 === report.expectedModel.sha256)
    ), { expected: report.expectedModel, actual: report.modelAssets });
  }
