/** Real-browser V3 acceptance. Run against a production preview. */
...
const output = path.resolve(process.env.VERIFY_OUTPUT || 'docs/redesign-v3-2026-09-08/verification.json');
...
(async () => {
  ...
  await broken.route('**/models/hamster-v3.glb',r=>r.abort());await broken.goto(report.url,{waitUntil:'networkidle'});
