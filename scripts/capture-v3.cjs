/** Capture the real V3 production page, preserving native video timing.
...
const output = path.resolve(process.env.CAPTURE_OUTPUT || 'docs/redesign-v3-2026-09-08/after');
const framesDir = fs.mkdtempSync(path.join(os.tmpdir(), 'hamster-v3-frames-'));
...
(async () => {
  ...
  log('Silver hero, actual idle');await page.waitForTimeout(1300);
  ...
  const buffer=fs.readFileSync('web/public/models/hamster-v3.glb');meta.model={file:'web/public/models/hamster-v3.glb',bytes:buffer.length,sha256:crypto.createHash('sha256').update(buffer).digest('hex')};
